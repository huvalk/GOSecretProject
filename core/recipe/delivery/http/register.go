...
func RegisterHTTPEndpoints(router *mux.Router, useCase recipeInterfaces.RecipeUseCase) {
	...
	router.Handle("/favorites/{id:[0-9]+}/add", http.HandlerFunc(h.AddToFavorites)).Methods("POST")
	router.Handle("/favorites/{id:[0-9]+}/delete", http.HandlerFunc(h.DeleteFromFavorites)).Methods("POST")
